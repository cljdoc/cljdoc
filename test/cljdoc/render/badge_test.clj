...
(defn- attributes [node]
  (reduce (fn [acc n]
          ...
          {}
          (.asList (.attributes node))))

(defn- select [soup selector]
  (.selectFirst soup selector))
...
(t/deftest wide-char-test
  (let [badge-text "Hello \u0604 there \u102A friend \u0FD0"
    ...
    (t/is (match? {:x "473" :y "148" :textLength "1834"}
                  (-> (select actual-soup (str "text:contains(" badge-text ")"))
                      attributes)))))
