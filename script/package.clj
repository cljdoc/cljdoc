...
(defn task
  ...
  (let [project-root (-> (tasks/shell {:out :string} "git rev-parse --show-toplevel") :out str/trim)
    ...
    (if-not (or force (not (fs/exists? zipfile)))
      ...
      (do
        ...
        (compile-js/task {})
        (compile-java/task {})
        (let [long-sha (-> (tasks/shell {:out :string} "git rev-parse HEAD") :out str/trim)]
