...
(defn ingest-git!
  ...
  [storage {:keys [project version scm-url pom-revision] :as _repo}]
  {:pre [(string? scm-url)]}
  (let [git-analysis (ana-git/analyze-git-repo project version scm-url pom-revision)]
    (if (:error git-analysis)
      {:scm-url scm-url :error (:error git-analysis)}
      (do
        (log/info "Importing Articles" scm-url pom-revision)
        (storage/import-doc
         storage
