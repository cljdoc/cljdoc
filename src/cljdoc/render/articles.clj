...
(defn doc-tree-view
  ...
  ([cache-id doc-bundle current-page indent?]
   (doc-tree-view cache-id doc-bundle current-page indent? 0))
  ([cache-id doc-bundle current-page indent? level]
   (when (seq doc-bundle)
     (->> doc-bundle
          (map (fn [doc-page]
                   ...
                   [:li
                    ...
                    [:a.link.blue.hover-dark-blue.dib
                     {:style {:word-wrap "break-word"}
                      ...
                      :class (str (when (= current-page slug-path) "fw7")
                                  (if indent? " pa1" " pv1"))}
                     (:title doc-page)]
                    (doc-tree-view cache-id (:children doc-page) current-page indent? (inc level))])))
          (into [:ul.list
                 {:class (str (when (pos? level) "f6-ns")
                              (if indent? " pl2" " pl0"))}])))))
