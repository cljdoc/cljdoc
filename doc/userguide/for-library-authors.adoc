...
After building your docs, they are available at https://cljdoc.org[cljdoc.org] for online and link:for-users.adoc#offline-docs[offline] browsing.
...
== Emojis
Cljdoc will render emojis found in Markdown (`.md`) and AsciiDoc (`.adoc`) articles.

You should see, for example, see :space_invader: for `:space_invader:`.
We use GitHub as our authority for the list of emojis and how they are rendered.

Emojis are not rendered in literal blocks or docstrings.

[TIP]
====
In AsciiDoc, a colon-prefixed word at the start of a line (e.g., `:space_invader:`) will be interpreted as a document attribute, not as an emoji.
To treat it as an emoji, you can use AsciiDoc's inline passthrough:

[source,adoc]
----
+:space_invader:+ will render as :space_invader:
----

Emojis that don't start at the beginning of a line can be used without passthrough.
====
...
* By hand. You can edit a `pom.xml` manually when cutting a release, but this is error-prone.
...
* https://leiningen.org/[Leiningen] will automatically create the `<scm>` entry for you if you set the
https://codeberg.org/leiningen/leiningen/src/commit/40227328d4a9c8945362d6d626d19c2449175df6/sample.project.clj#L509-L518[`:scm` in your `project.clj`].
For example:
+
[source,clojure]
----
...
----
+
** If you don't specify `:scm`, leiningen will infer scm values from your project's `~/.git` dir.
This is what https://github.com/juxt/bidi/blob/d94316235c4e59d7c44fca9fcc489b386f526bcc/project.clj#L1-L45[bidi] does.
This depends on a valid `git remote` being configured.
** An `:scm` `:tag` can be specified, but is not common practice.
A missing `:scm` `:tag` will be auto-filled with the `HEAD` git commit `sha`.
This is what https://github.com/juxt/bidi/blob/d94316235c4e59d7c44fca9fcc489b386f526bcc/project.clj#L1-L45[bidi] does by complete omission of `:scm`, and what many other projects do by omitting only `:tag`, like https://github.com/clj-commons/manifold/blob/402b5689defab0d7c5138fa591e859492481a247/project.clj#L6[manifold].
** If your library is not in your git root dir, use `:scm` `:dir`, for an example, see https://github.com/weavejester/cljfmt/blob/12f67c57f038bd7803cbbfe6b011d5a510fd8cb4/cljfmt/project.clj#L4[cljfmt].
...
You can choose to link:#rebuild[rebuild] against the current releases.
...
* By REST request
** An automated release process can trigger a build via rest link:/doc/docsets.adoc#trigger-a-build-for-a-project[trigger a cljdoc build via REST].
