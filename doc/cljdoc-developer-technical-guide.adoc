...
Being aware of how to diagnose and fix analysis job builds will also be of use, see link:fixing-builds.adoc[fixing cljdoc builds].
...
Logback logs on logging itself can be found in `log/logger.started-<YYYY-MM-dd>.log`.
There is no log rotation, but on cljdoc startup <YYYY-MM-dd> is replaced with the current date.
These logs include `ERROR` events when we the cljdoc sentry appender fails to log to sentry.io.
If you are on the ops team, it is a good idea to have an occassional peek a peek at these files on production.
...
If you use sdkman, we have an `.sdkmanrc`, use `sdk env`, so select dev version.
...
Our current approach is pretty hacky, but good enough for now.
Add any new test namespaces to `cljdoc.client.test-runner` namespace.

The test framework is very limited and implemented in `cljdoc.client.test-macros`.

=== Launching a Server Side REPL
...
[source,shell]
----
bb offline-docset --project bidi --version 2.1.3 --output bidi-2.1.3.zip
----

=== GitHub Emojis

Cljdoc renders GitHub emojis in md and adoc articles (but not docstrings).
For example, `:space_invader:` is rendered as :space_invader:.

The source of truth for the set of emojis and how they are rendered is GitHub.
To update this source of truth, run:

[source,shell]
----
bb update-github-emojis
----

Commit any changes to `resources/emojis.edn`.
