...
But, to answer the call to "reduce the number of requests", we now cache Maven Central results as edn under `./resources/maven-central-search/`.
...
If a developer notices a change to edn files under `./resources/maven-central-search/` they should commit and push it.
