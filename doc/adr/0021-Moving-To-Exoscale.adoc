...
Open questions:
* Currently adding ssh keys via terraform but completely manually for circle, (and eventually martin).
This is an awkward thing to remember to do after a terraform re-create of the compute instance.
But for now, I think we'll just document the awkwardness.
