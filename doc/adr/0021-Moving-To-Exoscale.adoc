...
- create `nomad` user for nomad service and and `consul` user for consul service
- use `/etc/nomad.d` and `/etc/consul.d` as home and config dirs
- use `/var/lib/nomad` and `/var/lib/consul` as data dirs

TODO: Oops: From https://developer.hashicorp.com/nomad/docs/operations/nomad-agent:
