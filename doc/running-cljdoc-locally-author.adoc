...
:example-project-version: 1.0.130
...
== Prerequisites
You'll need https://www.docker.com/products/docker-desktop/[Docker Desktop] or https://www.docker.com/get-started[plain old Docker].
Docker Desktop does better at <<volume-mapping>>.

== General Tips

[#volume-mapping,reftext=§volume mapping]
=== Volume Mappings
If you are using Docker Desktop, mapped volume files user ownership is to the host user.
You are all good.

If you are using plain old Docker, check your user id and group id:
[source,shell]
----
echo $(id -u):$(id -g)
----
The cljdoc docker container runs under user `cljdoc` that has a user id of `1000` nd group id of `1000`.
If your output matches `1000:1000`, all will work seamlessly.

Otherwise, you'll need to add `--user $(id -u):$(id -g)` to your docker launch command to align ownership of of mapped volumes to your host user.
Note that this may produce warnings about unwritable internal docker container files.
The cljddoc server should still function correctly.
...
docker run --rm \
  --publish 8000:8000 \
  --volume "$HOME/.m2:/home/cljdoc/.m2" \
  --volume ./.cljdoc-preview:/app/data \
  --platform linux/amd64 \
  {docker-image}
----
+
TIP: The `--volume "$HOME/.m2:/home/cljdoc/.m2"` mapping was updated in May of 2026 when we switched to running cljdoc from a non-root user (we used to map to `/root/.m2`).
...
[#importing-unpublished-library,reftext=§importing an unpublished library]
...
[#browse,reftext=§browse library docs]
...
[NOTE]
====
When testing locally, you may also want verify what your library docs look like as an offline docset: +
