...
:example-project-version: 1.0.53
...
The sections that follow include example command variants for running from cljdoc source and docker.
...
[source,shell,subs="verbatim,attributes"]
----
docker run --rm --entrypoint clojure {docker-image} -A:cli ingest --help
----
=====
====
...
If you want a full preview where direct GitHub references (links to images, source code, etc) work, then you'll also want to push your commits to GitHub.
...
TIP: If you have been testing {example-project-name} locally, be sure to delete it from your local maven repository.
...
You are in development mode and want to preview what your library docs will look like on cljdoc.

Normally cljdoc will look in your published library's `pom.xml` under `project/scm` tag to learn what GitHub `url` and `tag` it should use to reference articles and source code.
It can be convenient to not have to alter your `pom.xml` while testing changes during development.

If this is true for you, the cljdoc `ingest` command allows the `pom.xml`:

* `project/scm/url` to be overridden via `--git`
* `project/scm/tag` to be overridden via `--rev`
...
* `{example-project-version}` is the version of {example-project-name} published to your local maven repository.
* `pass:a[{example-project-import-url}]` is the GitHub URL for {example-project-name}, update if you have forked the repo.
=====
====
