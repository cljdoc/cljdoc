...
* https://adoptium.net/en-GB/temurin/releases/?version=23&package=jdk[Java Development Kit 23] - the cljdoc web server currently runs on JDK25 in production, so ideally, you'll want to be developing with JDK23 too.
...
The launch your REPL with `bb dev` connect to it from your favourite IDE, then navigate to `cljdoc.server.system` namespace, load it, then start the server:
+
[source,clojure]
----
